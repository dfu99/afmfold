...
A limitation of \Model is that it requires a priori definition of CVs as features to be estimated from the AFM image. If it is challenging to define appropriate CVs a priori, one could first perform structural sampling using methods such as AFlow, BioEmu, or MSA subsampling, and then identify meaningful CVs by applying dimensionality reduction techniques like PCA to extract important features. 
...
In the future, conditioning the prior (such as Boltz-2's physical potenal) of an ensemble generative model should enable integrated modeling that unifies experimental measurements, physical priors, and computational inference.
