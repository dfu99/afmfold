...
Specifically, while \AFiii employs a DDPM following the variance-preserving (VP) formulation, we compute the likelihood of the generated structure with respect to the specified CVs and add to the original \AFiii score a driving force that increases this likelihood. With this modified score, the generated structures becomes explicitly conditioned on the CVs.
...
Sampling the generative process of \AFiii can be written as the stochastic sampler of EDM \cite{karras2022elucidating}:
\begin{equation}
    \begin{split}
        &X_{t_{i+1}} = X_{t_i} - \eta (t_{i+1} - \hat{t}_i) \nabla_{X_{t_i}} \log p_{t_i}(X_{t_i} | a) + \lambda \sqrt{\hat{t}_i^2 - t_i^2} \epsilon, \\
        &\nabla_{X_{t_i}} \log p_{t_i}(X_{t_i} | a) \approx - \big( X_{t_i} - \text{Denoiser}(X_{t_i} | a) \big) / \hat{t}_i,
    \end{split}
    \label{eq:af3-sde}
\end{equation}
Here, $\hat{t}_i$ denotes a time slightly later than $t_i$, defined as $\hat{t}i = (1 + \gamma_i)t_i$, with a parameter $\gamma_i$ that is set to $0.8$ when $t{i+1} > 1.0$, and $0$ otherwise. 
...
\begin{equation}
    X_{t_{i+1}} = X_{t_i} - \eta (t_{i+1} - \hat{t}_i) \big( \nabla_{X_{t_i}} \log p_{t_i}(X_{t_i} | a) + \nabla_{X_{t_i}} \log p_{t_i}(\phi_{\text{target}} | X_{t_i}, a) \big) + \lambda \sqrt{\hat{t}_i^2 - t_i^2} \epsilon
    \label{eq:af3-guided}
\end{equation}
so that the process is guided toward $\phi(X_0) \approx \phi_{\text{target}}$, where $\phi: \R^{3m} \to \R^{D}$ is a differentiable mapping from structures to this feature space.
...
The optimal select of $\eta_t$ is analyzed in \Cref{app:guidance_sch}.
