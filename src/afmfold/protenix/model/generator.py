...
#   Changes include sample_diffusion.
# ---------------------------------------------------------
...
from afmfold.protenix.model.utils import centre_random_augmentation
from afmfold.protenix.model.guidance import DomainDistanceLoss, guidance_scale
...
def sample_diffusion(
    ...
    guidance_kwargs: Optional[Mapping[str, Any]] = None,
) -> torch.Tensor:
    ...
    def _chunk_sample_diffusion(chunk_n_sample, inplace_safe):
        ...
        # Modified in afmfold: add a restraint on the inter-domain distance.
        domain_loss = DomainDistanceLoss(
            model_path=guidance_kwargs.model_path, 
            image_path=guidance_kwargs.image_path, 
            domain_pairs=guidance_kwargs.domain_pairs, 
            manual=guidance_kwargs.manual, 
            in_nm=guidance_kwargs.in_nm,
            device=device,
            )
        
        for t, (c_tau_last, c_tau) in enumerate(
            ...
            current_time = t / (len(noise_schedule) - 1)
            
            if guidance_kwargs.t_start >= 0 and current_time >= guidance_kwargs.t_start:
                with torch.enable_grad():
                    x_coords = x_noisy.clone().detach().requires_grad_(True)

                    # Compute loss on the inter-domain distance.
                    loss = domain_loss.compute_loss(x_coords, guidance_kwargs.atom_array)
            ...
            eta = guidance_scale(t=current_time, t_start=guidance_kwargs.t_start, **(guidance_kwargs.scaling_kwargs))
            delta = delta + eta * g
